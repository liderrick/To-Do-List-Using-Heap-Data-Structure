...
* Name: Derrick Li
* Date: May 28, 2017
* Description: Part 1: Heap implementation - dynamicArray.c file
*/
...
void adjustHeap(DynamicArray* heap, int last, int position, compareFunction compare)
{
	...
	if (rightChildIndex < last)   /* If true, there are two children */
	{
		/* Get the index of the smaller of the two children */
		int smallestChildIndex;

		if (compare(dyGet(heap, leftChildIndex), dyGet(heap, rightChildIndex)) < 0)
		{
		...
		/* Compare value at position to value at smallestChildIndex, swap value if value at position is greater */
		if (compare(dyGet(heap, position), dyGet(heap, smallestChildIndex)) > 0)
		{
			dySwap(heap, position, smallestChildIndex);

			/* Recursively call adjustHeap and pass in smallestChildIndex as position argument */
			adjustHeap(heap, last, smallestChildIndex, compare);   
		}
	}
	else if (leftChildIndex < last) /* Else if this is true, there is only one child */
	{
		/* Compare value at position to value at leftChildIndex, swap value if value at position is greater */
		if (compare(dyGet(heap, position), dyGet(heap, leftChildIndex)) > 0)
		{
			dySwap(heap, position, leftChildIndex);

			/* recursively call adjustHeap and pass in leftChildIndex as position argument */
			adjustHeap(heap, last, leftChildIndex, compare);
		}
	}

	/* Else no children -- we are at the end of array, i.e. bottom of the heap. Done */
}
...
void dyHeapAdd(DynamicArray* heap, TYPE value, compareFunction compare)
{
	...
	int parent;
	int position = dySize(heap) - 1;   /* Get last index */

	while (position > 0)
	{
		parent = (position - 1) / 2;   /* Parent index of position */

		/* If value at position is less than value of parent, perform swap and update position to parent */
		if (compare(dyGet(heap, position), dyGet(heap, parent)) < 0)
		{
		...
		{
			/* If value at position is greater than value of parent, then heap order property is achieved and thus can exit function */
			return;   
		}
	}
}
